...
export default function SideberMenuItem({ typeMenu, isOpen, to, title, icon, dataMenu, index }: SideberMenuItem) {
    ...
    if (typeMenu == "main") {
        return (
            <div onClick={() => handleClick(to)} className={`flex items-center text-sm gap-3.5 font-medium p-2 hover:bg-blue-100 hover:scale-110 rounded-md ${isActive && `bg-blue-200`}`}>
                <div><i className={`${icon} fs-2`} /></div>
                <label
                    style={{ transitionDelay: `${index + 1}00ms` }}
                    className={`whitespace-pre duration-500 text-lg font-bold ${!isOpen && "opacity-0 translate-x-28 overflow-hidden"}`}
                >
                    {title}
                </label>
            </div>
    ...
    if (typeMenu == "sub") {
        return (
            <div onClick={() => handleClick(to)} className={`flex items-center text-sm gap-3.5 font-medium p-2 hover:bg-blue-100 rounded-md ${isActive && `bg-blue-200`}`}>
                <div><i className={`${icon} fs-2`} /></div>
